...
- [N05] Measure the listed files and the groups not audited before reading any of them, estimate the listed files' tokens as bytes divided by four, and estimate the audit stage's marginal cost as a range from that figure to three times it, since reading adds overhead to what the files weigh. Count the skill's fixed cost apart, about 30,000–40,000 tokens already paid once it loads — the skill's own body, the skill list and system reminders the session injects, and the round trips of its questions — and label every figure "tokens of context (estimate)":
