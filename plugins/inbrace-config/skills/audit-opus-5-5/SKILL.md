...
- [N39] Before reading the first batch, always ask one question per [N49], even when the plan has a single batch, naming what will be read — files and batches — the stage's marginal cost and the total expected to the end, and offering each pertinent combination of scope and run mode, then the quick sweep and cancel: full scope in this session; full scope in parallel subagents, only when the plan has more than one batch; reduced scope — memory files and their imports, rules, agents and settings — in this session, only when it saves at least one batch, with its own estimate and a warning that it leaves out the skills, where most text tuned for Opus 5 usually lives; reduced scope in parallel subagents, only when the reduced plan has more than one batch; the quick sweep of [N51]; or cancel, which goes straight to the close in [N45]. A subagent option finishes sooner and costs more, and states the tier its subagents run on per [N40], with the base context each one loads on start — the project memory measured in [N05] and the rules injected with it — per subagent and in total, at that tier. Where more than four options apply, the most `AskUserQuestion` takes, ask it as the numbered list of [N33]'s fallback, in the same order as the options table of [N07].
- [N40] Start each subagent, once the user approved them per [N49], on an explicit tier cheaper than the session's, such as `sonnet`, by passing `model` in the call and never inheriting the session's model, since classifying against a closed table needs no more; follow the project's own dispatch conventions — brief format, required fields — where its `CLAUDE.md` or hooks require them. Give each one its list of files, the pattern table, the Stage 2 norms and the project facts they depend on — the evaluation from [N38], the model pins, the read-only files — inline or in one shared file under `.claude/audits/` that each subagent reads, and have it return its findings as text in the [N09] line format and write no file; this session writes the findings file.
- [N46] Recommend one option in the [N39] question and state why: full scope in this session up to ten batches; above that, reduced scope in this session when its plan fits ten batches, and reduced scope in subagents when it still has more than ten; never recommend the quick sweep with ten batches or fewer.
