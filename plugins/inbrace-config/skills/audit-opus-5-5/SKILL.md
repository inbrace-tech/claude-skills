...
- [N09] Write the findings file at `.claude/audits/opus-5-5-<YYYY-MM-DD>.findings.md` under the root, one line per finding in the line format of the `<return_contract>` in `${CLAUDE_PLUGIN_ROOT}/agents/batch-auditor.md`, the one place that format is defined, reading it before writing the first finding, whether the batches run in this session, in subagents or as the quick sweep; the pattern id comes from the table below.
